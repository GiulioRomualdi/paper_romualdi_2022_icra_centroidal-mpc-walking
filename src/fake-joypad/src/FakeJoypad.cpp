...
struct FakeJoypad::Impl
{
    JoypadSignal joypad;
    // number of keys
    static constexpr int NUM_KEYS = SDLK_DOWN;
...
bool FakeJoypad::advance()
{
    ...
    if (SDL_PollEvent(&event))
    {
        ...
        if (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP)
        {
            ...
            m_pimpl->joypad.leftAnalogX = m_pimpl->keyState[SDLK_d] - m_pimpl->keyState[SDLK_a];
            m_pimpl->joypad.leftAnalogY = m_pimpl->keyState[SDLK_s] - m_pimpl->keyState[SDLK_w];
            m_pimpl->joypad.rightAnalogX
                = m_pimpl->keyState[SDLK_RIGHT] - m_pimpl->keyState[SDLK_LEFT];
            m_pimpl->joypad.rightAnalogY
                = m_pimpl->keyState[SDLK_DOWN] - m_pimpl->keyState[SDLK_UP];
